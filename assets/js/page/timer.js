...
function init(){
  ...
  $('#btn_timer_submit').click(function(e){
	...
	logger("parts[0] "+parts[0]);
	logger("parts[1] "+parts[1]);
	logger("parts[2] "+parts[2]);
    
    var par1 = (parseInt(parts[0]) * 3600);
    var par2 = (parseInt(parts[1]) * 60);
    var par3 = parseInt(parts[2]);
    logger("Par1 "+par1);
    logger("Par2 "+par2);
    logger("Par3 "+par3);
    
    var tots = par1 + par2 + par3;
    logger("Tots "+tots);

    totalHours = tots;
    logger("totalHours "+totalHours);
