...
if (!defined('User_class')) {
    ...
    class User extends databaseData
    {
        ...
        public function User($id, PDO $connection)
        {
            ...
            $query = "SELECT id, username, department, phone, email,last_name, first_name, pw_reset_code,can_add,can_checkin FROM {$GLOBALS['CONFIG']['db_prefix']}user WHERE id = :id";
			debug($query." - ".$id);
            $stmt = $connection->prepare($query);
            $stmt->execute(array(':id' => $this->id));
            $result = $stmt->fetch();
			#debug("User result ".json_encode($result));
            list(
                    $this->id,
                    $this->username,
                    $this->department,
